...
Nessa versão, foi adicionado um array chamado cardapio para armazenar os resultados intermediários da programação dinâmica. No final de cada iteração do loop, o lucro total até o dia atual é armazenado em cardapio[dia]. Além disso, a função planejarCardapio agora retorna o lucro total do cardápio.
...
A diferença principal está no uso do array cardapio na implementação com programação dinâmica. Nessa abordagem, armazenamos os resultados intermediários dos subproblemas em cardapio, em vez de recalculá-los a cada iteração. Isso evita a repetição de cálculos desnecessários e melhora a eficiência do algoritmo.
...
Ao introduzir a programação dinâmica, armazenamos os resultados intermediários dos subproblemas em cardapio. No loop principal, calculamos o lucro máximo para cada dia apenas uma vez e armazenamos em cardapio. Assim, nas iterações subsequentes, podemos simplesmente acessar o valor já calculado em vez de recalcular. Isso reduz a complexidade de tempo para O(dias * pratos) para apenas O(dias). No entanto, o custo espacial aumenta, pois precisamos armazenar os resultados intermediários em cardapio.
...
4. É criado um array chamado cardapio para armazenar os resultados intermediários da programação dinâmica. O tamanho desse array é definido como dias + 1, pois precisamos lidar com índices de 1 a dias.
...
11. Ao final de cada iteração do loop principal, o lucro total até o dia atual é armazenado no array cardapio no índice correspondente ao dia.
...
A programação dinâmica reduz a complexidade de tempo do algoritmo, evitando cálculos repetitivos e armazenando os resultados intermediários em cardapio. Isso melhora a eficiência do algoritmo em termos de tempo de execução.
...
Além disso, o problema possui sobreposição de subproblemas, o que significa que diferentes iterações do algoritmo podem realizar cálculos semelhantes. A programação dinâmica aborda esse problema armazenando os resultados intermediários em cardapio, permitindo a reutilização desses resultados nas iterações posteriores, em vez de recalculá-los repetidamente.
